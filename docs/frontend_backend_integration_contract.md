...
`forecast.bff.analysis_presentation` owns the presentation mapping. The canonical top-level order is `sales_quantity`, `sales_mix`, `sales_price`, `sales_fx`, `material_total`, `manufacturing_realized`, `inventory_timing`, `sga_variable`, `sga_fixed`, and `tariff`, followed by a separately classified Residual. Inventory Timing is independently sourced as Manufactured COGS Effect minus Current Manufacturing Cost Effect and is included exactly once. Customer freight remains inside sales price exactly once, tariff remains separate, the raw-material JPY component stays inside `material_total`, and no MCM effect is introduced. The mapper rejects any effect-set, amount, inventory source/scope status, unit, provenance, period, or `effects_total + residual = OP_delta` mismatch; it never plugs or recalculates a business amount.
...
The Fixed Cost taxonomy gate is resolved without changing the engine. `manufacturing_realized` includes variable manufacturing activity/unit effects and fixed manufacturing occurrence effects without an inventory-realization multiplier; the realization rate is reference-only. `inventory_timing` separately captures the manufactured-COGS/current-cost timing effect. `sga_fixed` is fixed SG&A only and excludes customer delivery freight and tariff. A new top-level Fixed Cost effect would therefore double-count fixed manufacturing.
...
The reduced canonical contract intentionally omits mock-only adjusted OP, invented product-group SG&A/OP allocations, annual plan targets, and narrative causes. It does not create backend business rules to preserve those fields. Migration 011 narrows Migration 010 selection to a strictly available default Result whose Comparison Model is the globally published default Model. No default means `EMPTY`; an arbitrary latest analysis pair is never selected. Worker completion creates the monthly dashboard snapshot while pinned workbooks are already open; Viewer/Admin dashboard requests never open Excel or execute an engine.
...
Still open after this slice: P&L mock-only fields without a canonical definition, shared production session/rate-limit stores, live Migration 010/011 validation, production worker and Forecast HTTP timeout budgeting, Golden business acceptance, cancel, and real progress/stage.

## Forecast React orchestration vertical-slice overlay

The trusted Forecast flow now uses the existing `forecast.engine.ForecastEngine` exactly as the Streamlit workflow does: an explicit published Base Model is downloaded and SHA-verified, each requested month is applied sequentially to the previous month Workbook, and only the final deterministic Workbook is registered. React collects canonical inputs and never implements sales, production, material, MCM, manufacturing, SG&A, tariff, or workbook formulas.

`POST /api/admin/forecasts` is session-, CSRF-, and Admin-protected. The V1 request pins `base_model_id`, year, ordered month range, the complete canonical sales/production/MCM product sets, signed cost adjustments, rates, and an actor-scoped idempotency key. Validation derives allowed product and adjustment identities from `model_mapping.json`; LC remains 4-inch/PCS, FS remains LENGTH/m, JPY is not rescaled, and MCM is not promoted to a separate material effect.

Additive Migration 011 adds a durable Forecast operation with advisory-lock idempotency, canonical payload plus release/mapping/engine fingerprint, Base ID/SHA, lease/recovery state, generated Model linkage, and operator cleanup RPCs. The same actor/key/payload replays one Model; a changed payload or pinned provenance conflicts. Final bytes are limited, hashed, uploaded to private `pnl-models/models/{model_id}/source.xlsx`, verified, and finalized only as `FORECAST`, unpublished, and non-default. A lost finalize response is recovered by operation ID; an uncertain commit never deletes a possibly referenced object. Publication remains the existing explicit Admin Model publication operation, after which the Forecast appears in Analysis options. Forecast generation does not auto-submit an analysis and does not alter the Viewer Dashboard.
