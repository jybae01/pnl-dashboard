  ...
  return <section className="pnl-report__table-container" data-testid="sga-table-shell">
    ...
    <div className="pnl-report__table-wrap">
      ...
      <table className="pnl-report__financial-table" style={{ width: tableWidth }} data-table-kind="sga" data-mode={mode} data-column-count={columnCount}>
        <thead>
          {mode === 'PLAN_ACTUAL_COMPARE' && <><tr><th rowSpan={2} style={{ width: 220 }}>판관비 항목</th><th rowSpan={2} style={{ width: 50 }}>구분</th><th colSpan={4} style={{ width: 440 }}>{selectedLabel === '6월' ? '당월 실적 비교 (6월)' : `${selectedLabel} 실적 비교`}</th><th colSpan={4} className="pnl-report__cumulative" style={{ width: 440 }}>누계 실적 비교 (1월 ~ {selectedLabel})</th></tr><tr><th style={{ width: 110 }}>{selectedLabel === '6월' ? '당월 계획' : `${selectedLabel} 계획`}</th><th style={{ width: 110 }}>{selectedLabel === '6월' ? '당월 실적' : `${selectedLabel} 실적`}</th><th style={{ width: 105 }}>차이</th><th style={{ width: 115 }}>증감률</th><th className="pnl-report__cumulative" style={{ width: 110 }}>누계 계획</th><th className="pnl-report__cumulative" style={{ width: 110 }}>누계 실적</th><th className="pnl-report__cumulative" style={{ width: 105 }}>누계 차이</th><th className="pnl-report__cumulative" style={{ width: 115 }}>증감률</th></tr></>}
          {mode === 'ACTUAL_ONLY' && <tr><th style={{ width: 210 }}>판관비 항목</th><th style={{ width: 54 }}>구분</th>{actualPeriodKeys.map((key) => <th key={key} style={{ width: 82 }}>{periods.find((period) => period.key === key)?.label ?? key}</th>)}<th className="pnl-report__cumulative" style={{ width: 95 }}>누계 실적</th></tr>}
        ...
        <tbody>{visibleRows.map((row) => {
          ...
          return <tr key={row.key} data-row-key={row.key} className={`row-${row.kind} row-sublevel-${row.level}`}>
            ...
            <td className="text-center pnl-report__unit-cell">{row.category}</td>
