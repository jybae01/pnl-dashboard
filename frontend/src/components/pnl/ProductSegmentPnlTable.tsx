...
export function ProductSegmentPnlTable({ segments, periods, actualPeriodKeys, initialPeriodKey, defaultCustomRangeKey }: ProductSegmentPnlTableProps) {
  ...
  const columnCount = mode === 'PLAN_ACTUAL_COMPARE' ? 10 : mode === 'ACTUAL_ONLY' ? 9 : 5;
  ...
  return <section className="pnl-report__table-container" data-testid="product-table-shell">
    <header className="pnl-report__table-toolbar">
      <div className="pnl-report__table-toolbar-primary">
        ...
        <div className="pnl-report__segmented pnl-report__segment-selector" role="group" aria-label="제품군 선택">{segments.map((item) => <button type="button" key={item.key} className={item.key === segment?.key ? 'active' : ''} data-unit={item.businessUnit} data-dimension={item.dimensionLabel} onClick={() => setSegmentKey(item.key)}>{item.label}</button>)}</div>
    ...
    {segment ? <div className="pnl-report__table-wrap">
      <div className="pnl-report__table-unit" style={{ width: tableWidth }}>(단위: 백만원, {segment.businessUnit}, 원, %)</div>
      <table className="pnl-report__financial-table" style={{ width: tableWidth }} data-table-kind="product" data-mode={mode} data-column-count={columnCount}>
        ...
        <tbody>{segment.rows.map((row) => {
          const expected = mode === 'PLAN_ACTUAL_COMPARE' ? 8 : mode === 'ACTUAL_ONLY' ? 7 : 3;
