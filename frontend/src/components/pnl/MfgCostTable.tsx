...
export function MfgCostTable({ rows, periods, actualPeriodKeys }: MfgCostTableProps) {
  const displayedPeriods = actualPeriodKeys.slice(0, 6);
  return <section className="pnl-report__table-container" data-testid="cogs-table-shell">
    ...
    <div className="pnl-report__table-wrap">
      <div className="pnl-report__table-unit" style={{ width: 1045 }}>(단위: 백만원, %)</div>
      <table className="pnl-report__financial-table" style={{ width: 1045 }} data-table-kind="cogs" data-column-count="15">
        <thead>
        ...
        <tbody>{rows.map((row) => <tr key={row.key} data-row-key={row.key} className={`row-${row.kind}`}>
          ...
          {displayCells(row.cells, 14).map((cell, index) => <ReportingValueCell key={index} cell={cell} />)}
        </tr>)}</tbody>
      </table>
    </div>
  </section>;
