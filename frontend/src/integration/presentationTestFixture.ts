      ...
      : code === 'manufacturing_realized'
        ...
        : code === 'sga_variable'
          ...
          : code === 'sga_fixed'
            ...
            : { kind: (code === 'material_total' ? 'material' : code === 'inventory_timing' ? 'inventory' : code === 'tariff' ? 'tariff' : 'sales') as 'sales' | 'material' | 'inventory' | 'tariff', available: true, rows: [{ row_id: code === 'sales_price' ? 'sales_price:customer_delivery_transport' : `${code}:1`, label: code === 'sales_price' ? '고객배송 운반비' : code === 'tariff' ? '관세' : `${code} source`, unit: 'KRW', baseline: null, comparison: null, delta: null, profit_effect: effectValues[index], note: 'persisted source', section: code === 'tariff' ? 'selling' as const : null }], unavailable_reason: null },
  }));
