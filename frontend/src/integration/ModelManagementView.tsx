...
import { Check, ChevronDown, Clipboard, Database, FileUp, LockKeyhole, RefreshCw, Search, ShieldCheck, X } from 'lucide-react';
...
export function ModelManagementView({ onNavigateToForecast, onNavigateToAnalysis }: ModelManagementViewProps) {
  ...
  return <div className="data-management" aria-labelledby="management-page-heading">
    ...
    <section className="data-management__list-card" aria-labelledby="management-list-heading">
      ...
      {visibleListState === 'READY' && <div className="data-management__table-wrap"><table className="data-management__table"><thead><tr><th>모형</th><th>구분</th><th>적용 기간</th><th>버전</th><th>공개 상태</th><th>등록일</th><th>작업</th></tr></thead><tbody>{filteredModels.map((model) => <tr key={model.model_id} className={selectedModel?.model_id === model.model_id ? 'is-selected' : ''} onClick={() => setSelectedModel(model)}><td><button type="button" className="data-management__model-link" onClick={(event) => { event.stopPropagation(); setSelectedModel(model); }}>{model.display_name}<small>{model.file_name}</small></button></td><td><span className={`data-management__scenario data-management__scenario--${modelTypeClass(model.model_type)}`}>{modelTypeLabel(model.model_type)}</span></td><td className="tabular-nums">{periodLabel(model)}</td><td className="tabular-nums">{model.version}</td><td><span className={`data-management__publication ${model.is_published ? 'is-published' : ''}`}>{model.is_published ? '공개' : '비공개'}</span>{model.is_default && <span className="data-management__default">기본</span>}</td><td className="tabular-nums">{dateLabel(model.uploaded_at)}</td><td><button type="button" className="data-management__small-action" onClick={(event) => { event.stopPropagation(); setSelectedModel(model); }}>상세</button></td></tr>)}</tbody></table></div>}
    </section>
