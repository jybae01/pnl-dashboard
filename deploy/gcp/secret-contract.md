...
| --- | --- | --- | --- |
| `pnl-supabase-secret-key` | web, worker, maintenance | `supabase_secret_key` | `SUPABASE_SECRET_KEY_FILE` |
...
The three dedicated runtime service accounts receive
`roles/secretmanager.secretAccessor` only on the exact secrets listed above.
The application does not use `GOOGLE_APPLICATION_CREDENTIALS`, service-account
key files, or any Google management API. Browser assets receive no Supabase
