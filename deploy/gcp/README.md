...
The verifier requires all 27 files in lexical order and their frozen SHA-256
...
backend-selected bootstrap) ->
`202608210001_forecast_tariff_metadata_finalize_v11.sql` (v1.1 Forecast tariff
metadata finalization) ->
`202608210002_analysis_monthly_fx_idempotent_v11.sql` (v1.1 monthly sales-FX
idempotent submission boundary). The verifier's expected count is 27, and a
passing result must report `migration_count` 27 with
`202608210002_analysis_monthly_fx_idempotent_v11.sql` as the latest migration.
...
against these exact 27 files; this verification step must not apply migrations.
...
history proving 27/27 with no gap or
