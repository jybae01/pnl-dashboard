...
The verifier requires all 26 files in lexical order and their frozen SHA-256
...
metadata finalization). The verifier's expected count is 26, and a passing
result must report `migration_count` 26 with
`202608210001_forecast_tariff_metadata_finalize_v11.sql` as the latest migration.
...
against these exact 26 files; this verification step must not apply migrations.
...
history proving 26/26 with no gap or
