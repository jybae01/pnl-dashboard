...
if not st.session_state.authenticated:
    ...
    st.markdown("<h2 style='text-align: center; color: #1F2937;'>📊 손익 데이터 모니터링</h2>", unsafe_allow_html=True)
...
.custom-tbl th { 
    ...
    background-color: rgba(128, 128, 128, 0.05); color: inherit; white-space: nowrap; 
}
.custom-tbl td { 
...
def clean_multiindex_html(html):
    if '<th>항목</th>' in html:
        html = html.replace('<th>항목</th>', '<th rowspan="2" style="vertical-align: middle; border-bottom: 1px solid rgba(128,128,128,0.2);">항목</th>', 1)
        html = re.sub(r'<th[^>]*>(?:&nbsp;|\s*)</th>', '', html, count=1)
        html = re.sub(r'<th[^>]*>Unnamed[^<]*</th>', '', html)
    return html

def render_html_table(df, is_compare=False):
    mode_class = " compare-mode" if is_compare else ""
    html = df.to_html(index=False, classes=f"custom-tbl{mode_class}", escape=False)
    html = clean_multiindex_html(html)
    html = html.replace("\n", "").replace("\r", "")
    wrapper = f'<div style="width:100%; overflow-x:auto;">{html}</div>'
    st.markdown(wrapper, unsafe_allow_html=True)

def render_pnl_table(df, is_compare=False):
    mode_class = " compare-mode" if is_compare else ""
    html = df.to_html(index=False, classes=f"custom-tbl{mode_class}", escape=False)
    html = clean_multiindex_html(html)
    html = html.replace("\n", "").replace("\r", "")
    wrapper = f'<div class="pnl-container" style="width:100%; overflow-x:auto;"><input type="checkbox" id="toggle-qty" style="display:none;"><input type="checkbox" id="toggle-sales" style="display:none;"><input type="checkbox" id="toggle-cogs" style="display:none;">{html}</div>'
    st.markdown(wrapper, unsafe_allow_html=True)
...
items = [
    ...
    '매출원가율', '매출총이익', '매출총이익률', '판관비', '영업이익', '영업이익률'
]
...
actual_rows = [
    ...
    sga_total_a, op_actual, op_ratio_a
]
plan_rows = [
    sales_total_p, sales_prod_p, sales_semi_p, sales_md_p, sales_etc_p, sales_inc_p, 
    ...
    sga_total_p, op_plan, op_ratio_p
]
...
is_comp_mode = (view_mode == "계획/실적 비교 보기")
...
if not is_comp_mode:
    ...
    render_pnl_table(df_table, False)
else:
    tuples = [('항목', '')]
...
render_html_table(df_cogs, True)
st.markdown("<br>", unsafe_allow_html=True)
...
is_comp_sga = (view_mode_sga == "계획/실적 비교 보기")
...
if not is_comp_sga:
    ...
    render_html_table(df_sga, False)
else:
    tuples_sga = [('항목', '')]
    ...
    render_html_table(df_sga_comp, True)
...
def render_type_table(rows_a, sums_a, rows_p, sums_p, view_mode):
    is_comp_type = (view_mode == "계획/실적 비교 보기")
    if not is_comp_type:
